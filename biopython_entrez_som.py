...
faculty_df = pd.read_excel('som_faculty.xlsx', sheet_name='senate_faculty')
faculty_df.head()
...
for index, row in faculty_df.iterrows():
    search_term = row["Faculty_Author_Affiliation_Orcid"]
...
output_file = 'pubmed_mesh_terms_som.csv'
faculty_df.to_csv(output_file, index=False)

## Load the saved CSV file as faculty_df
output_file = 'pubmed_mesh_terms_som.csv'
combined_faculty_df = pd.read_csv(output_file)
...
combined_faculty_df = combined_faculty_df[['Faculty_Full_Name', 'pub_mesh_terms']]
combined_faculty_df.rename(columns={'pub_mesh_terms': 'Combined_Mesh_Terms'}, inplace=True)
combined_faculty_df.head(10)
...
def remove_terms_from_string(terms_string, terms_to_remove):
    if pd.isna(terms_string) or terms_string is None or str(terms_string).strip() == '':
        return ''
...
unique_terms_df.to_excel('som_faculty_unique_mesh_terms.xlsx', index=False)
...
combined_faculty_df.to_excel('som_faculty_mesh_terms.xlsx', index=False)
...
pca_matrix = combined_faculty_df.drop(combined_faculty_df.columns[1:6], axis=1)
pca_matrix.head()

# Save PCA matrix to Excel
pca_matrix.to_excel('som_faculty_mesh_terms_matrix.xlsx', index=False)