...
print("Before TF-IDF transformation:")
print(f"Matrix shape: {feature_matrix.shape}")
print(f"Mean of first column: {feature_matrix.iloc[:, 0].mean():.3f}")
print(f"Mean of last 10 columns: {feature_matrix.iloc[:, -10:].mean().mean():.3f}")

# Apply TF-IDF transformation
tfidf = TfidfTransformer(norm='l2', use_idf=True)
feature_matrix_tfidf = tfidf.fit_transform(feature_matrix)

# Convert back to DataFrame
feature_matrix_tfidf = pd.DataFrame(
...
print(f"Matrix shape: {feature_matrix_tfidf.shape}")
print(f"Mean of first column: {feature_matrix_tfidf.iloc[:, 0].mean():.3f}")
print(f"Mean of last 10 columns: {feature_matrix_tfidf.iloc[:, -10:].mean().mean():.3f}")

# Replace the original feature_matrix with TF-IDF version
feature_matrix = feature_matrix_tfidf
